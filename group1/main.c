...
static void test_find_repeat_byte_xor();
static void test_detect_ecb();
...
/*
 * Test the find_repeated_byte_xor function
 */
static void test_find_repeat_byte_xor()
{
    const size_t num_candidates = sizeof candidates / sizeof candidates[0];
    const char *winner = find_repeated_byte_xor(candidates, num_candidates);
    size_t raw_size = strlen(winner) / 2;
    uint8_t decrypted[raw_size + 1];
    read_base16(decrypted, winner, raw_size * 2);
    uint8_t key = detect_repeated_byte_xor(decrypted, raw_size);
    repeated_byte_xor(key, decrypted, decrypted, raw_size);
    decrypted[raw_size] = '\0';
    const char expected[] = "Now that the party is jumping\n";
    assert(strcmp((char *) decrypted, expected) == 0);
    printf("Find repeat byte xor test passed!\n");
}

/*
 * Test the is_ecb_encrpyted function
 */
const char* ecb_candidates[] = {
...
static void test_detect_ecb()
{
    const size_t num_ecb_candidates = sizeof ecb_candidates /
        sizeof ecb_candidates[0];
    uint32_t sum = 0;
    const char *winner = NULL;
    for (size_t i = 0; i < num_ecb_candidates; ++i) {
        size_t raw_size = strlen(ecb_candidates[i]) / 2;
        uint8_t raw[raw_size];
        read_base16(raw, ecb_candidates[i], raw_size * 2);
        uint32_t is_ecb = is_ecb_encrypted(raw, raw_size);
        if (is_ecb)
            winner = ecb_candidates[i];
        sum += is_ecb;
    }
    const char expected[] = "";
    assert(sum == 1);
    assert(strcmp(winner, expected) == 0);
    printf("Detect ecb test passed!\n");
}
